...
%% 
% Our model contains quite a few variables
...
% to the internal plasma current profile
...
% flux map
% * *y_type* are the output _names and indexes_
%% 
...
% use of Matlab's PDE-toolbox, so the toolbox command are used in the plot_plasma 
% function and in this script)

figure
nnodes = size(model.Input_struct.p,2); % no. of FEM nodes
plot_plasma(model.Input_struct, model.x_np(1:nnodes), 50);
%% 
% and then we add the plasma boundary

hold on
hb = pdecont(model.Input_struct.p, model.Input_struct.t, model.x_np(1:nnodes), psib*[1 1]); % plot the level contour at psib
...
%% 
% The model can be recast in standard I-S-U form by putting $x=I$ and setting
%% 
% * $A = -L^{-1}R$
% * $B = L^{-1}S$
% * $E = -L^{-1}LE$
...
figure;
plot_plasma(model.Input_struct);
hold on
plot(r_fg,z_fg,'.b')
xlim([-2.00 8.45])
ylim([-5.00 4.30])
%% 
% We can plot the equilibrium value of these virtual sensors as follows
...
figure;
plot_plasma(model.Input_struct, model.x_np(1:nnodes), linspace(min(eq_fg/2/pi),max(eq_fg/2/pi),30));
hold on
...
dI(5) = -500;
%% 
% and finally compute the variation of the outputs as $\delta y = C\delta I$
